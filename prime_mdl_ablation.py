...
def generate_prime_gap_data():
    ...
    X = []
    Y = []
    for i in range(6, len(gaps)):
        X.append([
            gaps[i-1], gaps[i-2], gaps[i-3], gaps[i-4], gaps[i-5], gaps[i-6]
        ])
        Y.append(gaps[i])
...
def run_prime(X_train, Y_train, X_val, Y_val, lambda_penalty=0.005):
    pop_size = 2048
    seq_len = 15
    generations = 5000
    ...
    val_pred = evaluate_rpn(best_seq_overall, X_val)
    val_mse = float('inf')
    if val_pred is not None:
        val_mse = torch.mean((val_pred - Y_val)**2).item()
        
    print(f"LAMBDA: {lambda_penalty}")
    print(f"Final Eq: {best_eq_overall}")
    print(f"Complexity: {len(best_eq_overall)}")
    print(f"Train MSE: {best_train_mse:.6f}")
    print(f"Test MSE:  {val_mse:.6f}")
    print("-" * 40)
    
if __name__ == "__main__":
    X_train, Y_train, X_val, Y_val = generate_prime_gap_data()
    print("--- RUNNING WITH COMPLEXITY PENALTY (MDL ON) ---")
    run_prime(X_train, Y_train, X_val, Y_val, lambda_penalty=0.005)
    print("--- RUNNING WITHOUT COMPLEXITY PENALTY (MDL OFF) ---")
    run_prime(X_train, Y_train, X_val, Y_val, lambda_penalty=0.0)
