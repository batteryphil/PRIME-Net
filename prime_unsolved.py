...
def generate_prime_gap_data():
    print("Generating first 10,000 prime numbers to extract Prime Gaps...")
    primes = get_primes(10000)
    gaps = np.diff(primes).astype(np.float32)
    
    # Normalize gaps somewhat to keep math stable (max gap in first 10,000 primes is usually < 100)
    gaps = gaps / 100.0
...
def run_prime():
    pop_size = 4096
    seq_len = 15
    generations = 100000 # Run all day
    
    X_train, Y_train = generate_prime_gap_data()
    ...
    print("Beginning ALL DAY PRIME Search on the Unsolved Prime Gaps Problem...")
    ...
    for gen in range(generations):
        ...
        if current_mse < best_train_mse and current_mse > 0.0:
            ...
            print(f"Gen {gen:6d} | MSE: {best_train_mse:8.4f} | Eq: {best_eq_overall}", flush=True)
