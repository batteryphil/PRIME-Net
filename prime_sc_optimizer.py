...
def run_prime_numba(name, X_full, Y_full, lambda_penalty=0.005, generations=3000, seed=42, T_max=0.0, target_seq=None):
    ...
    master_idx  = rng.integers(0, VOCAB_SIZE, SEQ_LEN, dtype=np.int32)
    vote_buffer = np.zeros(SEQ_LEN,  dtype=np.float64)
    step_sizes  = np.ones(SEQ_LEN,   dtype=np.int32)
    last_signs  = np.zeros(SEQ_LEN,  dtype=np.int32)

    mut_scale        = 3
    target_flip_rate = 0.05
    threshold        = 10.0
    best_train_mse   = float('inf')
    best_seq         = master_idx.tolist()
    best_eq          = ""
    ...
    for gen in range(generations):
        p    = rng.integers(-mut_scale, mut_scale+1, (pop_size, SEQ_LEN), dtype=np.int32)
        mask = (rng.random((pop_size, SEQ_LEN)) < 0.20).astype(np.int32)
        p   *= mask
        p[0] = 0   # elitism
        ...
        fits = eval_population_cpu(pop_idx, X_train, Y_train, np.float32(lambda_penalty))

        std_fit = float(fits.std())
        advs = rng.standard_normal(pop_size) if std_fit < 1e-8 else \
               (fits - fits.mean()) / (std_fit + 1e-8)

        vote_buffer += ((p.astype(np.float64) / mut_scale) * advs[:,None]).sum(axis=0)

        # Simulated Annealing Injection
        if cur_temp > T_min and T_max > 0.0:
            noise = rng.standard_normal(SEQ_LEN)
            vote_buffer += noise * cur_temp
        ...
        if 0 < cur_mse < best_train_mse:
            ...
            best_seq = pop_idx[best_i].tolist()
        ...
        # ======================================================================
        # NEW: SYNTACTICALLY VALID SUBTREE CROSSOVER (PRIME-SC)
        # ======================================================================
        num_cross = 100
        children = np.full((num_cross, SEQ_LEN), 19, dtype=np.int32)
        valid_children = 0
        
        for c in range(num_cross):
            p1 = rng.integers(0, pop_size)
            p2 = rng.integers(0, pop_size)
            seq1 = pop_idx[p1]
            seq2 = pop_idx[p2]
            ...
            if len1 > 0 and len2 > 0:
                ...
                if valid:
                    for s in range(st1 + len1, SEQ_LEN):
                        if seq1[s] != 19:
                            if c_idx >= SEQ_LEN: valid = False; break
                            child[c_idx] = seq1[s]
                            c_idx += 1
                ...
                if valid:
                    children[valid_children] = child
                    valid_children += 1
                    
        if valid_children > 0:
            c_fits = eval_population_cpu(children[:valid_children], X_train, Y_train, np.float32(lambda_penalty))
            best_c = int(c_fits.argmax())
            c_mse = float(-c_fits[best_c])
            
            if 0 < c_mse < best_train_mse:
                best_train_mse = c_mse
                best_seq = children[best_c].tolist()
                best_eq  = rpn_to_str(best_seq)
                stagnant_count = 0
                
                # Elitist injection: The child becomes the new swarm center
                master_idx = children[best_c].copy()
                vote_buffer.fill(0.0)
                step_sizes.fill(1)
        # ======================================================================
        ...
        if gen % 100 == 0:
            ...
            if target_seq is not None:
                hamming_dists = np.sum(pop_idx != target_seq_arr, axis=1)
