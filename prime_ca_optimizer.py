...
@njit(fastmath=True)
def get_rpn_subtree(seq, end_idx):
    if seq[end_idx] == 19:
        return -1, 0
    if seq[end_idx] <= 9:
        return end_idx, 1
    ...
    for i in range(end_idx, -1, -1):
        ...
        if t == 19: continue
        if t <= 9: needed -= 1
        elif t in (10,11,12,13,15): needed += 1
        
        if needed == 0:
            return i, (end_idx - i + 1)
            
    return -1, 0


def run_prime_numba(name, X_full, Y_full, lambda_penalty=0.005, generations=3000, seed=42, T_max=0.0, target_seq=None, num_islands=1, pop_size=2048, migration_type=None, forced_diversity=False, runway_gens=0):
    ...
    if forced_diversity:
        for k in range(num_islands):
            for i in range(SEQ_LEN):
                if i % 2 == 0:
                    if k % 4 == 0: master_idx[k, i] = rng.integers(0, 3) 
                    elif k % 4 == 1: master_idx[k, i] = rng.integers(3, 6) 
                    elif k % 4 == 2: master_idx[k, i] = rng.integers(6, 10) 
                    else: master_idx[k, i] = rng.integers(0, 10) 
                else:
                    if k % 4 == 0: master_idx[k, i] = rng.integers(10, 12) 
                    elif k % 4 == 1: master_idx[k, i] = rng.integers(12, 14) 
                    elif k % 4 == 2: master_idx[k, i] = rng.integers(14, 19) 
                    else: master_idx[k, i] = rng.integers(10, 19)
    else:
        for k in range(num_islands):
            for i in range(SEQ_LEN):
    ...
    for gen in range(generations):
        ...
        # Migration Event
        if gen > 0 and gen % 500 == 0 and migration_type is not None and num_islands > 1:
            telemetry['migration_events'] += num_islands
            successes = 0
            new_masters = master_idx.copy()
            
            for k in range(num_islands):
                neighbor = (k + 1) % num_islands
                
                if migration_type == "whole":
                    eval_pop = np.array([master_idx[k], master_idx[neighbor]], dtype=np.int32)
                    m_fits = eval_population_cpu(eval_pop, X_train, Y_train, np.float32(lambda_penalty))
                    
                    if m_fits[0] > m_fits[1]:
                        new_masters[neighbor] = master_idx[k].copy()
                        vote_buffer[neighbor].fill(0.0)
                        step_sizes[neighbor].fill(1.0)
                        successes += 1
                        
                elif migration_type == "subtree":
                    seq1 = master_idx[k]
                    seq2 = master_idx[neighbor]
                    
                    idx1 = rng.integers(0, SEQ_LEN)
                    st1, len1 = get_rpn_subtree(seq1, idx1)
                    idx2 = rng.integers(0, SEQ_LEN)
                    st2, len2 = get_rpn_subtree(seq2, idx2)
                    
                    if len1 > 0 and len2 > 0:
                        child = np.full(SEQ_LEN, 19, dtype=np.int32)
                        c_idx = 0
                        valid = True
                        
                        for s in range(st2):
                            if seq2[s] != 19:
                                if c_idx >= SEQ_LEN: valid = False; break
                                child[c_idx] = seq2[s]
                                c_idx += 1
                                
                        if valid:
                            for s in range(st1, st1 + len1):
                                if seq1[s] != 19:
                                    if c_idx >= SEQ_LEN: valid = False; break
                                    child[c_idx] = seq1[s]
                                    c_idx += 1
                        ...
                        if valid:
                            for s in range(st2 + len2, SEQ_LEN):
                                if seq2[s] != 19:
                                    if c_idx >= SEQ_LEN: valid = False; break
                                    child[c_idx] = seq2[s]
                                    c_idx += 1
                        ...
                        if valid:
                            # ─── REFINEMENT RUNWAY (COMPOSITION ARENA) ───
                            runway_pop_size = 128
                            ...
                            c_master_idx  = child.copy()
                            c_threshold   = 10.0
                            
                            freeze_mask = np.ones(SEQ_LEN, dtype=np.int32)
                            freeze_mask[st2:st2+len1] = 0
                            ...
                            for c_gen in range(runway_gens):
                                ...
                                c_fits = eval_population_cpu(c_pop_idx, X_train, Y_train, np.float32(lambda_penalty))
                                ...
                                c_threshold = max(5.0, c_threshold * (1.0 + (c_rate - target_flip_rate)))
                                
                            eval_pop = np.array([c_master_idx, seq2], dtype=np.int32)
                            m_fits = eval_population_cpu(eval_pop, X_train, Y_train, np.float32(lambda_penalty))
                            
                            if m_fits[0] > m_fits[1]:
                                new_masters[neighbor] = c_master_idx
                                vote_buffer[neighbor].fill(0.0)
                                step_sizes[neighbor].fill(1.0)
                                successes += 1
            
            master_idx = new_masters
            telemetry['successful_migrations'] += successes
