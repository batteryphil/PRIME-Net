...
def run_prime(X_hist, Y_true, generations=2000):
    pop_size = 512
    seq_len = 15
...
def generate_synthetic_data(domain_scale, noise_std, num_points=1000):
    # Base X is Uniform(-d, d)
    X = torch.empty(num_points, dtype=torch.float32, device=device).uniform_(-domain_scale, domain_scale)
    ...
    # True non-linear equation: sin(X[t-1]) + X[t-2]
    Y = torch.sin(X_hist[0]) + X_hist[1]
        
    # Inject sensor noise (N(0, sigma))
    if noise_std > 0:
...
def main():
    domains = [0.1, 0.5, 1.0, 2.0, 3.14159, 5.0]
    noises = [0.0, 0.01, 0.05, 0.1, 0.25, 0.5]
    num_seeds = 5
    generations = 2000
    
    out_file = 'phase_matrix_results.csv'
    write_header = not os.path.exists(out_file)
    
    print("========== PRIME PHASE BOUNDARY MATRIX ==========\n")
    print(f"Target: sin(X[t-1]) + X[t-2]")
    print(f"Domains: {domains}")
    print(f"Noises: {noises}")
    print(f"Seeds per cell: {num_seeds}")
    
    with open(out_file, mode='a', newline='') as f:
        writer = csv.writer(f)
        if write_header:
            writer.writerow(['Domain', 'Noise', 'Seed', 'EvolvedEquation', 'FinalMSE'])
            
        for d in domains:
            for noise in noises:
                for seed in range(num_seeds):
                    # Deterministic seed for reproducible data generation per run
                    torch.manual_seed(1000 * int(d*10) + int(noise*100) + seed)
                    
                    X_hist, Y_noisy = generate_synthetic_data(d, noise)
                    
                    final_mse, final_eq = run_prime(X_hist, Y_noisy, generations=generations)
                    
                    writer.writerow([d, noise, seed, final_eq, f"{final_mse:.4f}"])
                    f.flush()
                    print(f"[D={d:.2f} | N={noise:.2f} | S={seed}] Eq: {final_eq} (MSE: {final_mse:.4f})")
