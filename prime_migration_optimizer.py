...
def run_prime_numba(name, X_full, Y_full, lambda_penalty=0.005, generations=3000, seed=42, T_max=0.0, target_seq=None, num_islands=1, pop_size=2048, migration_type=None, forced_diversity=False):
            ...
            for k in range(num_islands):
                ...
                if migration_type == "whole":
                    ...
                    if len1 > 0 and len2 > 0:
                        ...
                        if valid:
                            eval_pop = np.array([child, seq2], dtype=np.int32)
                            m_fits = eval_population_cpu(eval_pop, X_train, Y_train, np.float32(lambda_penalty))
                            
                            if m_fits[0] > m_fits[1]:
                                new_masters[neighbor] = child
                                vote_buffer[neighbor].fill(0.0)
                                step_sizes[neighbor].fill(1.0)
                                successes += 1
